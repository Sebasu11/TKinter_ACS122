...
AUTHENTICATE_SECTOR_0 = [0xFF, 0x86, 0x00, 0x00, 0x05, 0x01, 0x00, 0x1C, 0x61, 0x00]
...
def bg_nopuedesalir():
        img = ImageTk.PhotoImage(Image.open(image_path7))
        panel.configure(image=img)
        panel.photo = img
        buzzer.on()
        app.after(TimepoVuelvaPronto,bg_ini)
...
def URL(Barcode):
        url= ("https://zoologicosantafe.matipos.com/API/AccesoV01/?codigoEntrada="+Barcode+"&direccionMAC="+macWithout)
        #req = Request(url)
        print (url)
        try:
            ...
            if card not in self.cards:
                ...
                if (sw1_s==144 and sw2_s==0):
                    ...
                    try:
                        ...
                        print ('Error reading this motherfucking thing')
                else:
